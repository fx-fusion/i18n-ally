...
export default class HistoryTreeProvider
  ...
  > = new vscode.EventEmitter<HistoryItem | undefined>();
  readonly onDidChangeTreeData: vscode.Event<HistoryItem | undefined> = this
    ._onDidChangeTreeData.event;
  /* tslint:enable*/

  private currentHistoryFile: string = "";
  private currentHistoryPath: string = "";
  private historyFiles: Record<string, any> = {}; // {yesterday: IHistoryFileProperties[]}
  ...
  initLocation() {
    vscode.commands.executeCommand('setContext', 'i18n-ally.local-history:treeLocation', HistorySettings.getTreeLocation());
  }
      ...
      this.controller
        ...
        .then(findFiles => {
          ...
          if (this.contentKind === EHistoryTreeContentKind.Current) {
            const historyFile = this.controller.decodeFile(
              fileName.fsPath,
              settings
            );
            // @ts-ignore
            ...
            switch (this.contentKind) {
              ...
              case EHistoryTreeContentKind.Search:
                ...
                if (keys && keys.length) {
                  ...
                  keys.forEach(key =>
                    items.push(
                      ...this.historyFiles[key].map((item: any) => item.file)
                    )
                  );
                  this.controller
                    .deleteFiles(items)
  ...
  public delete(element: HistoryItem): void {
    if (element.kind === EHistoryTreeItem.File)
      this.controller
        .deleteFile(element.file.fsPath)
        .then(() => this.refresh());
    else if (element.kind === EHistoryTreeItem.Group) {
      this.controller
        .deleteFiles(
          // @ts-ignore
          this.historyFiles[element.label].map(
            (value: IHistoryFileProperties) => value.file
          )
        )
        .then(() => this.refresh());
    }
  ...
  public compareToCurrent(element: HistoryItem): void {
    if (element.kind === EHistoryTreeItem.File) {
      ...
      this.controller.compare(
        element.file,
        vscode.Uri.file(this.currentHistoryFile),
        // @ts-ignore
        null,
        currRange
      );
  ...
  public select(element: HistoryItem): void {
    if (element.kind === EHistoryTreeItem.File) {
      ...
      // @ts-ignore
      this.tree[element.grp].grp.collapsibleState =
    ...
    if (element.kind === EHistoryTreeItem.File) {
      if (this.selection)
        ...
        vscode.window.showErrorMessage(
          "Select a history files to compare with"
        );
    }
  }
  ...
  constructor(
    ...
    switch (this.kind) {
      case EHistoryTreeItem.File:
        ...
        // @ts-ignore
        if (showIcon) this.iconPath = false;
        break;
    ...
    if (provider.contentKind === EHistoryTreeContentKind.Current) {
      this.command =
        this.kind === EHistoryTreeItem.File
          ? {
              command: "i18n-ally-treeLocalHistory.compareToCurrentEntry",
      ...
      this.command =
        this.kind === EHistoryTreeItem.File
          ? {
              command: "i18n-ally-treeLocalHistory.showEntry",
