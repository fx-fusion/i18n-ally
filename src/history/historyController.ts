...
import { IHistorySettings, HistorySettings } from "./historySettings";
...
export class HistoryController {
  ...
  public showAll(editor: vscode.TextEditor) {
    this.internalShowAll(
      this.actionOpen,
      editor,
      this.getSettings(editor.document.uri)
    );
  }
  ...
  public findAllHistory(
    ...
    noLimit?: boolean
  ): Promise<IHistoryFileProperties> {
    return new Promise((resolve, reject) => {
      // @ts-ignore
      if (!settings.enabled) resolve();

      let fileProperties = this.decodeFile(fileName, settings, true);
      this.getHistoryFiles(
        // @ts-ignore
        fileProperties && fileProperties.file,
        settings,
        noLimit
      )
        .then(files => {
  ...
  public findGlobalHistory(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (findFile)
        this.findAllHistory(find, settings, noLimit).then(fileProperties =>
          // @ts-ignore
          resolve(fileProperties && fileProperties.history)
        );
      else
    ...
    if (!isOriginal && settings.saveDelay) {
      if (!this.saveBatch.get(document.fileName)) {
        ...
        return this.timeoutPromise(
          this.internalSaveDocument,
          settings.saveDelay * 1000,
          [document, settings]
        );
  ...
  private internalSaveDocument(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!settings.absolute) {
        revisionDir = path.dirname(
          this.getRelativePath(document.fileName).replace(/\//g, path.sep)
        );
      } else {
        revisionDir = this.normalizePath(
      ...
      const revisionPattern = this.joinPath(
      ...
      if (isOriginal) {
        ...
        // (Often the case...)
        const files = glob.sync(revisionPattern, {
          cwd: settings.historyPath.replace(/\\/g, "/")
        });
        // @ts-ignore
        ...
        if (timeout && timeout.isTimedOut()) {
          vscode.window.showErrorMessage(
            `Timeout when internalSave: ' ${document.fileName}`
          );
      ...
      const revisionFile = this.joinPath(
        settings.historyPath,
        revisionDir,
        p.name,
        p.ext,
      ...
      if (
        this.mkDirRecursive(revisionFile) &&
        this.copyFile(document.fileName, revisionFile, timeout)
      ) {
  ...
  private allowSave(
    ...
    if (
      ...
      anymatch(settings.exclude, docFile)
    )
      return false;
  ...
  private internalShowAll(
    ...
    me.findAllHistory(document.fileName, settings).then(fileProperties => {
  ...
  private actionOpen(values: IHistoryActionValues, editor: vscode.TextEditor) {
    return this.internalOpen(
      vscode.Uri.file(values.selected),
      // @ts-ignore
      editor.viewColumn
    );
  }

  private actionCompareToActive(
    values: IHistoryActionValues,
    editor: vscode.TextEditor
  ) {
    return this.internalCompare(
      vscode.Uri.file(values.selected),
      vscode.Uri.file(values.active)
    );
  }

  private actionCompareToCurrent(
    values: IHistoryActionValues,
    editor: vscode.TextEditor,
    settings: IHistorySettings
  ) {
    return this.internalCompare(
      vscode.Uri.file(values.selected),
      this.findCurrent(values.active, settings)
    );
  }

  private actionCompareToPrevious(
    values: IHistoryActionValues,
    editor: vscode.TextEditor
  ) {
    if (values.previous)
      return this.internalCompare(
        vscode.Uri.file(values.selected),
        vscode.Uri.file(values.previous)
      );
  }
    ...
    if (filePath)
      return new Promise((resolve, reject) => {
        vscode.workspace.openTextDocument(filePath).then(
          d => {
            vscode.window.showTextDocument(d, column).then(
              // @ts-ignore
              () => resolve(),
              err => reject(err)
            );
  ...
  private internalCompare(
    ...
    if (file1 && file2) {
      ...
      let title =
        path.basename(file1.fsPath) + "<->" + path.basename(file2.fsPath);
  ...
  private internalDecodeFile(
    ...
    if (filePath.includes("/.history/") || filePath.includes("\\.history\\")) {
      //startsWith(this.settings.historyPath))
      isHistory = true;
      ...
      if (index) {
        ...
        // @ts-ignore
      } else return null; // file in history with bad pattern !
    }
    ...
    if (history != null) {
      ...
      if (history !== isHistory) {
        if (history === true) {
          ...
          else p.dir = this.normalizePath(p.dir, false);
        } else {
          // if (history === false)
  ...
  private findCurrent(
    ...
    // @ts-ignore
    if (fileProperties !== null) return vscode.Uri.file(fileProperties.file);
      ...
      Promise.all(fileNames.map(file => this.internalDeleteFile(file)))
        .then(results => {
          ...
          results.some((item: any) => {
            if (item.err) {
              vscode.window.showErrorMessage(
                `Error when delete files history: '${item.err}' file '${item.fileName}`
              );
              return true;
            }
          });
  ...
  private purge(
    ...
    me.getHistoryFiles(pattern, settings, true).then(files => {
      if (!files || !files.length) {
        return;
      }
      ...
      for (let file of files) {
        ...
        if (stat && stat.isFile()) {
          endTime =
            stat.birthtime.getTime() + settings.daysLimit * 24 * 60 * 60 * 1000;
          if (now > endTime) {
            fs.unlinkSync(file);
          }
        }
  ...
  private copyFile(source: string, target: string, timeout?: Timeout): boolean {
    try {
      ...
      if (timeout && timeout.isTimedOut()) {
        vscode.window.showErrorMessage(
          `Timeout when copyFile: ' ${source} => ${target}`
        );
        return false;
      }
      // @ts-ignore
      ...
      vscode.window.showErrorMessage(
        // @ts-ignore
        `Error with copyFile: '${err.toString()} ${source} => ${target}`
      );
      return false;
    }
  }
  ...
  private normalizePath(dir: string, withDrive: boolean) {
    if (process.platform === "win32") {
      if (!withDrive) return dir.replace(":", "");
